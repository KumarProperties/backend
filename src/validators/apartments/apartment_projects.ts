...
import {
	...
	ProjectRequestBody,
	ApartmentStatus,
} from '../../models/apartments/apartment_projects';
import { FlatView } from '../../schema/apartments/flat_view';
import { TechStack, TechStackEntry } from '../../schema/apartments/tech_stack';
import {
	Amenities,
	Bulleting,
	FlatDetails,
	IsometricView,
	Specification,
} from '../../schema/apartments/flat_details';
import { ApartmentDocument } from '../../schema/apartments/document';
import { Certifactions } from '../../models/apartments/certifications';
import { MapSet, LocationType, MapEntry } from '../../schema/apartments/map';
import { MetaTag } from '../../schema/apartments/apartments';
...
const CertificationValidationSchema = Joi.object<Certifactions>({
...
const MapEnty = Joi.object<MapEntry>({
	distance: Joi.string().optional().allow(''),
	location_name: Joi.string().optional().allow(''),
});
const MapSetValidationSchem = Joi.object<MapSet>({
	title: Joi.string().optional().allow(''),
	type: Joi.string()
		.optional()
		.valid(...Object.values(LocationType))
		.optional(),
	locations: Joi.array().items(MapEnty.optional()).required(),
});
...
const apartmentAdditionValidation = Joi.object<ApartmentAdditionRequest>({
	...
	maps: Joi.array().items(MapSetValidationSchem.optional()).optional(),
